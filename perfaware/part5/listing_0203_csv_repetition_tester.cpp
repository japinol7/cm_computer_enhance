...
{
    repetition_value_type TargetValueType;
    u64 TargetValue;
    u64 CPUTimerFreq;
    u64 TryForTime;
    u64 TestsStartedAt;
...
static void ComputeDerivedValues(repetition_value *Value, u64 CPUTimerFreq)
{
    ...
    if(CPUTimerFreq)
    {
        ...
        f64 Giga = (1000.0 * 1000.0 * 1000.0);
        PerCount[StatValue_gbPerSecond] = PerCount[RepValue_ByteCount] / (Gigabyte * Seconds);
        PerCount[StatValue_GOpPerSecond] = PerCount[RepValue_OpCount] / (Giga * Seconds);
    }
...
static void PrintValue(char const *Label, repetition_value Value)
{
    ...
    printf(" %fGOp/s", Value.PerCount[StatValue_GOpPerSecond]);
...
static void NewTestWave(repetition_tester *Tester, repetition_value_type TargetValueType, u64 TargetValue, u64 CPUTimerFreq, u32 SecondsToTry = 10)
{
    if(Tester->Mode == TestMode_Uninitialized)
    {
        Tester->Mode = TestMode_Testing;
        Tester->TargetValueType = TargetValueType;
        Tester->TargetValue = TargetValue;
    ...
    else if(Tester->Mode == TestMode_Completed)
    {
        ...
        if((Tester->TargetValueType != TargetValueType) ||
           (Tester->TargetValue != TargetValue))
        {
...
inline void CountBytes(repetition_tester *Tester, u64 ByteCount)
{
    repetition_value *Accum = &Tester->AccumulatedOnThisTest;
    Accum->E[RepValue_ByteCount] += ByteCount;
}

inline void CountOps(repetition_tester *Tester, u64 OpCount)
{
    repetition_value *Accum = &Tester->AccumulatedOnThisTest;
    Accum->E[RepValue_OpCount] += OpCount;
}
...
static b32 IsTesting(repetition_tester *Tester)
{
    if(Tester->Mode == TestMode_Testing)
    {
        ...
        if(Tester->OpenBlockCount) // NOTE(casey): We don't count tests that had no timing blocks - we assume they took some other path
        {
            ...
            if(Accum.E[Tester->TargetValueType] != Tester->TargetValue)
            {
...
static void NewTestWave(repetition_test_series *Series,
                        repetition_tester *Tester, repetition_value_type TargetValueType, u64 TargetValue,
                        u64 CPUTimerFreq, u32 SecondsToTry = 10)
{
    ...
    NewTestWave(Tester, TargetValueType, TargetValue, CPUTimerFreq, SecondsToTry);
}
